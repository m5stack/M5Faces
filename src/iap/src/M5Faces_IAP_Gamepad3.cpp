 ...
 *
 * M5Faces_IAP_Gamepad3.cpp — Gamepad3 专用 IAP 实现
 * Device-specific IAP implementation for M5Faces_Gamepad3.
 *
 * 参考 / Reference:
 *   examples/faces-keyboard3-test/src/BOOTLOADER_UPGRADE_F0.cpp
 *   (与 KEYBOARD3 同一固件工程师，协议相似，但此文件为 Gamepad3 的独立完整副本，
 *    方便后续 Gamepad3 协议单独变化时无需顾虑 KEYBOARD3。)
 *
 * 协议:
...
static const char *TAG = "IAP_GAMEPAD3";

/* ------------------------------------------------------------------ */
/* 触发引导加载程序                                                      */
/* Trigger the bootloader.                                             */
/* 与 KEYBOARD3 同一固件工程师，需要相同的 SDA/SCL 拉低序列               */
/* ------------------------------------------------------------------ */
static esp_err_t gamepad3_trigger(faces_iap_bus_handle_t bus, const FacesIapOptions &opts)
{
    ...
    /* Step 2: Pull SDA/SCL LOW for 100ms (same as KEYBOARD3, same firmware engineer)
       Critical: Must disconnect I2C peripheral from GPIO matrix first so that
       gpio_set_level() actually drives the physical pins. */
...
/* ------------------------------------------------------------------ */
static esp_err_t gamepad3_stream(faces_iap_bus_handle_t bus, const FacesIapOptions &opts)
{
    esp_err_t ret;
...
esp_err_t m5faces_iap_gamepad3_run(faces_iap_bus_handle_t bus, const FacesIapOptions &opts)
{
    ...
    if (iap_probe(bus, FACES_IAP_BOOT_ADDR, 200) == ESP_OK) {
        ...
        if (opts.progress_cb) opts.progress_cb(0, "Gamepad3: Bootloader already active", opts.progress_ctx);
    } else {
        if (opts.progress_cb) opts.progress_cb(0, "Gamepad3: Triggering bootloader...", opts.progress_ctx);

        ESP_LOGI(TAG, "Trigger bootloader (reg 0xFD=1 on addr 0x%02X)", iap_eff_app_addr(opts));
        ret = gamepad3_trigger(bus, opts);
        if (ret != ESP_OK) return ret;

        if (opts.progress_cb) opts.progress_cb(0, "Gamepad3: Waiting for bootloader...", opts.progress_ctx);
    ...
    ret = gamepad3_stream(bus, opts);
    if (ret != ESP_OK) return ret;

    if (opts.progress_cb) opts.progress_cb(99, "Gamepad3: Waiting for app...", opts.progress_ctx);
    ...
    if (opts.progress_cb) opts.progress_cb(99, "Gamepad3: Verifying...", opts.progress_ctx);

    ESP_LOGI(TAG, "Gamepad3 IAP complete");
    return ESP_OK;
}
